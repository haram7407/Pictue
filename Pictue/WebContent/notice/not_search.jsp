...
<div id = 'notice_frm'>
	...
	<div>
		...
		<div class = 'items'>
		  <c:forEach var = 'vo' items ='${list}'>
			<div class ='item' onclick = 'not.view(${vo.serial})'>
	...
	<div id = 'btn_wrap'>
	  <c:if test="${page.startPage>1 }">
		<input type = 'button' value = '처음' onclick = 'not.move(1)'/>
		<input type = 'button' value = '이전' onclick = 'not.move(${page.startPage-1})'/>
	  </c:if>
	  
	  <c:forEach var='p' begin='${page.startPage }' end='${page.endPage }'>
	  	<input type = 'button' value='${p }' onclick='not.move(${p})'/>
	  </c:forEach>
	  
	  <c:if test="${page.endPage<page.totPage }">
		<input type = 'button' value = '다음' onclick = 'not.move(${page.endPage+1})'/>
		<input type = 'button' value = '맨끝' onclick = 'not.move(${page.totPage})'/>
	  </c:if>
	</div>
</div>
<script>
