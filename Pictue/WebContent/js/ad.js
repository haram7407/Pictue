...
ad.init = function(){
	...
	$('#frm_ad #btnFind').on('click', function(){
		var frm = $('#frm_ad')[0];
		frm.nowPage.value = 1;
		var param = $(frm).serialize();
		$('#content').load('./ad.do?job=search', param);
	})
	
	$('#frm_ad #btnSearch').on('click', function(){
		var frm = $('#frm_ad')[0];
	...
	$('#frm_ad #btnInsert').on('click', function(){
		var frm = $('#frm_ad')[0];
		var param = $(frm).serialize();
		$('#content').load('./ad/ad_register.jsp', param);
	})
	
	$('#frm_ad #btnInsertR').on('click', function(){
	...
	$('#frm_ad #btnModify').on('click', function(){
		var frm = $('#frm_ad')[0];
		var param = $(frm).serialize();
		$('#content').load('./ad.do?job=modify', param);	
	})

	$('#frm_ad #btnUpdate').on('click', function(){
	...
	$('#frm_ad #btnDelete').on('click', function(){
		var yn = confirm("삭제하시겠습니까?");
		if(!yn){return;} else {
		var frm = $('#frm_ad')[0];
		var param = $(frm).serialize();
		$('#content').load('./ad.do?job=delete', param)
		}
	})
}
...
ad.view = function(serial){
	...
	var param = $(frm).serialize();
	$('#content').load('./ad.do?job=view', param);
