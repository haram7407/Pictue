...
var brd = {};
brd.init = function(){
	$('#comu #btnInsert').on('click', function(){
		var frm = $('#frm_comu')[0];
		var param = $(frm).serialize();
		$('#comuboard').load('comu_register.jsp', param);
	})
	
	$('#comu #btnsearch').on('click', function(){
		var frm = $('#frm_comu')[0];
		var param = $(frm).serialize();
		$('#comuboard').load('comu_search.jsp', param);
	})
	
	$('#comu #btnupdate').on('click', function(){
		var frm = $('#frm_comu')[0];
		var param = $(frm).serialize();
		$('#comuboard').load('comu_modify.jsp', param);
	})
...
*/
	...
	function funcFind(){
		...
		var url='comu_suggest_result.jsp'
	...
	sel.ondblclick = function(){ //select에서 더블클릭한 경우
		...
		var url = 'comu_suggest_detail.jsp?eid=' + v;
		xhr.open('get', url);
		xhr.send();
		xhr.onreadystatechange = function(){
			if(xhr.status==200 && xhr.readyState==4){
